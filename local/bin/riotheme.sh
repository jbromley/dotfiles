...
THEMES_DIR="$HOME/.config/rio/themes"
CONFIG="$HOME/.config/rio/config.toml"
...
if [[ ! -f "$CONFIG" ]]; then
    echo "Rio config not found: $CONFIG" >&2
    exit 1
fi
...
themes_list=$(find "$THEMES_DIR" -maxdepth 1 -name '*.toml' -printf '%f\n' | sed 's/\.toml$//' | sort)
